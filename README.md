...
1. 启动时候的需要同时启动mysql和redis，账户密码可以根据配置文件自行选择。

2. 启动前需要构建eblog数据库(暂时不支持自动创建)，并且在数据库下面执行sql脚本。
由于项目启动配置文件默认选择application-dev文件，因此数据库默认账户密码是root@123;支持自定义。

3. 下面是配置文件中redis的启动环境，支持自定义。
>   redis:
    database: 0
    host: 127.0.0.1
    port: 6379
